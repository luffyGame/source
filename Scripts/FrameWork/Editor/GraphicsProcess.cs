...
{
    public static class GraphicsProcess
    {
        ...
        //[MenuItem("FrameWork/Graphics/IncludeBuiltinShaders", false, 11)]
        private static void IncludeBuiltinShaders()
        {
            ...
            foreach (Object asset in AssetDatabase.LoadAllAssetsAtPath("Resources/unity_builtin_extra")
                ...
                .Where(obj => !obj.name.StartsWith("Hidden") && !obj.name.StartsWith("Legacy Shaders") 
                     && !obj.name.StartsWith("VR") && !obj.name.StartsWith("AR") 
                     && !obj.name.StartsWith("Nature"))
