...
namespace Ionic.Zip
{
    ...
    public partial class ZipEntry
    {
        ...
        {
            if (_container.ZipFile != null && _container.ZipFile.Verbose) _container.ZipFile.Log(string.Format(format, args));
        }
        ...
        private void InternalExtract(string baseDir, Stream outstream, string password)
        {
            ...
            finally
            {
                if (_ioOperationCanceled)
                {
                    WriteStatus("warning:extract {0} caneled", targetFileName);
                    if (targetFileName != null)
                    {
                        try
                        {
